...
export default function CatInfoBar({ species, name, weightKg, onSettings }: CatInfoBarProps) {
  ...
  return (
    <button
      type="button"
      className="flex w-full min-h-11 items-center gap-3 rounded-2xl bg-card px-4 py-3 text-left shadow-sm touch-manipulation active:bg-surface"
      aria-label={`${speciesLabels[species]} ${name}，${weightKg} 千克，打开设置`}
      onClick={onSettings}
    >
      <SpeciesIcon className="size-6 shrink-0 text-accent" aria-hidden />
